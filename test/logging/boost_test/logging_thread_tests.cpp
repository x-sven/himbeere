...
/* test case*/
void test_case_logging_fwrite_with_thread()
{
    using namespace __fwrite__;

    float fl_difftime = 0.;

    cout << "Starting test using fwrite with thread." << endl;
    ...
    timelog.open ("fwrite_thread_timelog.log", ios::out);
    timelog << "time_s" << "\t"
            << "time_us" << "\t"
            << "runtime_ms"
            << "\n";

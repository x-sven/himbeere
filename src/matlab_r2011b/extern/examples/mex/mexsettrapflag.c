...
mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
    ...
    if (nrhs !=1 || !mxIsDouble(prhs[0]) ||
        ...
        mexErrMsgIdAndTxt( "MATLAB:mexsettrapflag:invalidInput",
                "Input argument must be a real scalar double.");
    }
 
    if(nlhs > 1){
        mexErrMsgIdAndTxt( "MATLAB:mexsettrapflag:maxlhs",
    ...
    if(status==0) {
      mexPrintf("???? nofcn exists ????\n");
        ...
        mxFree(buf);
        mexErrMsgIdAndTxt( "MATLAB:mexsettrapflag:callmatlabFailed",

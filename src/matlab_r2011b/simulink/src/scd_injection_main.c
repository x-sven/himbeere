...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    /* Store intdimensions in user data to be able to deallocate */
...
{
    /* In this function, we will populate permament settings in work vector
    ...
    mxGetString(IOTYPE_ARG(S),insertiontype,80);
	  ...
	  * we do not have to run these if's at each time step */

...
//    m_pJoystick = SDL_GameControllerOpen(joy_index);
//printf("Controller is mapped as \"%s\".", SDL_GameControllerMapping(m_pJoystick));
SDL_GameController *ctrl;
for (int i = 0; i < SDL_NumJoysticks(); ++i) {
    if (SDL_IsGameController(i)) {
        printf("Index \'%i\' is a compatible controller, named \'%s\'", i, SDL_GameControllerNameForIndex(i));
        ctrl = SDL_GameControllerOpen(i);
        printf("Controller %i is mapped as \"%s\".", i, SDL_GameControllerMapping(ctrl));
    } else {
